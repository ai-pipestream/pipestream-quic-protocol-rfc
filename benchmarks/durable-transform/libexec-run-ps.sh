...
# TEST-ONLY fault-suite plumbing (recorded by the invoking suite script).
[ -n "${PS_KILL_AT:-}" ] && COORD_KILL+=(--test-kill-at "$PS_KILL_AT")
[ -n "${PS_SKIP_DECLARE:-}" ] && COORD_KILL+=(--test-skip-declare "$PS_SKIP_DECLARE")
COORD_SWAP=()
