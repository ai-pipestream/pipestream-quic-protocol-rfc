...
| `V2Main client` operations | `capabilities binding declare admit replay lookup unresolved watch page checkpoint manifest select read retry cancel skip cancel-scope complete detach`; every invocation reopens the journal and requires `--journal --authority --owner --creation-sequence` (Rust-compatible). `next-sequence` takes only connection arguments. |
| `V2Main client` recovery (Section 12.2.1) | Default `--retry-budget 0` is the one-shot mode: recovery is the next invocation, which replays the journaled intent under the original identity. `--retry-budget N [--retry-backoff-ms M]` (M default 200, doubling to 30 s with jitter) re-invokes the same journaled operation on a fresh connection after an authority `LIMIT_EXCEEDED`/`NOT_READY`/`WAIT_TIMEOUT`/`INTERNAL_ERROR`, a `CONTROL_RESET`, transport loss or the local request deadline; `CONFLICT`, `UNAUTHORIZED`, framing/integrity codes, terminal outcomes and any local or journal failure stop at once. Each retry prints `RECOVERING attempt=… budget=… code=… wait-ms=…`; exhaustion prints `UNRESOLVED attempts=… last=… budget-exhausted` (or `not-recoverable`) and exits 1 with the intent still journaled (`unresolved` lists it). |
| `V2Main` failure output | Every failure exits 1. An authority refusal (correlated REFUSAL or coded application close) additionally prints `REFUSED code=<CODE> detail=<peer diagnostic>` on stdout for the driver; local failures reach stderr only. `capabilities` prints `CAPABILITIES offered-idle-ms=… offered-lifetime-ms=… selected-idle-ms=… selected-lifetime-ms=… stream-limit=… pending-limit=… object-limit=…` (Section 12.1 diagnostics). |
| REFUSAL detail | The Java authority now carries its local bound or check label in the detail (`input receive deadline`, `owner connection ceiling`, `truncated payload`, …) exactly as the Rust authority does; it is bounded, non-normative and never parsed by the Java client. |
