...
# A 90s settle between kill and resume: the killed coordinator's
# connections need server-side reaping (past the 60s authority idle
# timeout), otherwise the resume plus the merge reader trips the
# 4-connections-per-principal ceiling and the reader connect is
# refused. Production crash supervision restarts slower than that
# anyway; see README (F3).
